...
class Campo:
    def __init__(self, ancho=800, alto=600):
        ...
        self.S_donor_threshold = 2.5 # Se inicializa aquí, pero será actualizado por la UI
    ...
    def actualizar_posiciones(self):
        ...
        fuerzas de atracción/repulsión basadas en S e I, intercambio de información,
        y fuerza de expulsión para nodos de alta entropía.
        ...
        I_max_transfer_threshold = 1.5 # Tope para recibir Información (AJUSTADO: más bajo)
        S_donor_threshold = self.S_donor_threshold # Usar el valor de la UI
        ...
        for i, nodo1 in enumerate(self.nodos):
            for j, nodo2 in enumerate(self.nodos):
                ...
                if distance < info_exchange_distance:
                    ...
                    # Transferencia de S: de S más alto a S más bajo
                    transfer_S_amount = abs(nodo1.S - nodo2.S) * exchange_rate * influence_factor
                    
                    if nodo1.S > nodo2.S: # S tiende a fluir de nodo1 a nodo2
                        if nodo2.S < S_donor_threshold: # nodo2 solo recibe S si no ha alcanzado su umbral de donante
                            new_S_vals[nodo1] -= transfer_S_amount
                            new_S_vals[nodo2] += transfer_S_amount
                        # else: nodo2.S >= S_donor_threshold, nodo2 no puede recibir S
                    else: # S tiende a fluir de nodo2 a nodo1
                        if nodo1.S < S_donor_threshold: # nodo1 solo recibe S si no ha alcanzado su umbral de donante
                            new_S_vals[nodo2] -= transfer_S_amount
                            new_S_vals[nodo1] += transfer_S_amount
                        # else: nodo1.S >= S_donor_threshold, nodo1 no puede recibir S

                    # Transferencia de I: de I más alto a I más bajo, con tope
                    delta_I = nodo1.I - nodo2.I
                    transfer_I_amount = abs(delta_I) * exchange_rate * influence_factor

                    if delta_I > 0: # I tiende a fluir de nodo1 a nodo2
                        available_I_space_nodo2 = max(0, I_max_transfer_threshold - nodo2.I)
                        actual_transfer = min(transfer_I_amount, available_I_space_nodo2)
                        new_I_vals[nodo1] -= actual_transfer
                        new_I_vals[nodo2] += actual_transfer
                    else: # I tiende a fluir de nodo2 a nodo1
                        available_I_space_nodo1 = max(0, I_max_transfer_threshold - nodo1.I)
                        actual_transfer = min(transfer_I_amount, available_I_space_nodo1)
                        new_I_vals[nodo1] += actual_transfer
                        new_I_vals[nodo2] -= actual_transfer
        ...
        # --- Calcular el 'Centro de Información' del clúster (para la fuerza de expulsión) ---
        center_x_I = 0.0
        center_y_I = 0.0
        total_I_for_center = sum(n.I for n in self.nodos) # Usar I para ponderar el centro

        if total_I_for_center > 0:
            for nodo in self.nodos:
                center_x_I += nodo.x * nodo.I
                center_y_I += nodo.y * nodo.I
            center_x_I /= total_I_for_center
            center_y_I /= total_I_for_center
        else: # Si no hay nodos con I, el centro es el centro del canvas
            center_x_I = self.ancho / 2
            center_y_I = self.alto / 2

        # --- Cálculo de fuerzas entre nodos (usando los S e I actualizados) ---
        fuerzas = {nodo: {'fx': 0.0, 'fy': 0.0} for nodo in self.nodos}

        repulsion_base_strength = 0.008  # FUERZA DE REPULSIÓN INCREMENTADA
        attraction_base_strength = 0.00008 # FUERZA DE ATRACCIÓN INCREMENTADA

        min_repulsion_distance = 50 
        max_attraction_distance = 300 
        
        S_expulsion_threshold = 2.0 # Umbral de S para activar la fuerza de expulsión
        expulsion_strength_from_core = 0.006 # Fuerza base de expulsión del núcleo (INCREMENTADA DRAMÁTICAMENTE)

        for i, nodo1 in enumerate(self.nodos):
            # 1. Fuerzas inter-nodo (atracción/repulsión)
            for j, nodo2 in enumerate(self.nodos):
                if i == j:
                    continue
                ...
                repulsion_factor = (nodo1.S + nodo2.S) # Suma de entropías para repulsión
                attraction_factor = (nodo1.I + nodo2.I) # Suma de informaciones para atracción
            ...
            # 2. Fuerza de Expulsión del Núcleo para Nodos de Alta Entropía
            if nodo1.S > S_expulsion_threshold:
                dx_from_center = nodo1.x - center_x_I
                dy_from_center = nodo1.y - center_y_I
                dist_from_center = math.hypot(dx_from_center, dy_from_center)
                
                if dist_from_center == 0:
                    dist_from_center = 0.1 # Evitar división por cero
                
                # La fuerza de expulsión es más fuerte cuanto mayor es S por encima del umbral
                # y disminuye al alejarse del centro de información
                expulsion_factor = (nodo1.S - S_expulsion_threshold) * (1 / max(1.0, dist_from_center)) * expulsion_strength_from_core 
                
                fuerzas[nodo1]['fx'] += (dx_from_center / dist_from_center) * expulsion_factor
                fuerzas[nodo1]['fy'] += (dy_from_center / dist_from_center) * expulsion_factor
...
class MotorN:
    ...
    def _dibujar_nodo(self, nodo):
        ...
        Si el nodo es un 'donante' de entropía (S >= S_donor_threshold), su contorno será punteado.
        """
        ...
        # Determinar el estilo del contorno para los nodos donantes de entropía
        outline_width = 1
        outline_dash = () # Línea sólida por defecto
        if nodo.S >= self.campo.S_donor_threshold: # Si S del nodo es >= al umbral de donante
            outline_width = 2
            outline_dash = (4, 2) # Línea punteada (4 píxeles llenos, 2 vacíos)
        ...
        self.canvas.create_oval(
            ...
            x + r, y + r,
            fill=nodo_color,
            outline="white",  # El contorno principal del nodo será blanco
            width=outline_width, 
            dash=outline_dash 
        )
        
        if r > 8:
            fill_color = "white" if nodo.ratio_realidad < 1.5 else "black" 
    ...
    def actualizar_metricas(self):
        ...
        s_espacial = self.campo.entropia_espacial() 
        
        if self.lbl_energy:
            self.lbl_energy.config(text=f"Total S: {total_S:.2f}\nTotal I: {total_I:.2f}")
            
        if self.lbl_entropy:
            self.lbl_entropy.config(text=f"Entropía Espacial: {s_espacial:.2f}")
        ...
        try:
            ...
            if root and hasattr(root, 'title'):
                root.title(f"Simulador S-I | Nodos: {len(self.campo.nodos)} | Sum S: {total_S:.1f} | Sum I: {total_I:.1f}")
        except Exception:
            pass
...
class App:
    def __init__(self, root):
        ...
        # Ajustar grid_columnconfigure para 3 columnas para velocidad y tope S
        frame_controles.grid_columnconfigure(0, weight=1) # para el label
        frame_controles.grid_columnconfigure(1, weight=2) # para el slider
        frame_controles.grid_columnconfigure(2, weight=1) # para el label de valor

        # Control de Velocidad
        ttk.Label(frame_controles, text="Velocidad:", background="#2b2b2b", foreground="white").grid(row=0, column=0, padx=5, sticky="w")
        ...
        self.lbl_velocidad_val = ttk.Label(frame_controles, text=f"{self.velocidad.get():.1f}x", background="#2b2b2b", foreground="white")
        self.lbl_velocidad_val.grid(row=0, column=2, padx=5, sticky="w")

        # Nuevo Control de Tope de Entropía (S)
        ttk.Label(frame_controles, text="Tope Entropía (S):", background="#2b2b2b", foreground="white").grid(row=1, column=0, padx=5, sticky="w", pady=5)
        self.s_donor_threshold_var = tk.DoubleVar(value=1.8) # Valor inicial más bajo para mayor efecto
        ttk.Scale(
            frame_controles,
            from_=0.1, # Min S
            to=3.0,   # Max S
            orient=tk.HORIZONTAL,
            variable=self.s_donor_threshold_var,
            command=self.actualizar_s_donor_threshold,
            length=150
        ).grid(row=1, column=1, padx=5, sticky="ew", pady=5)
        self.lbl_s_donor_threshold_val = ttk.Label(frame_controles, text=f"{self.s_donor_threshold_var.get():.1f}", background="#2b2b2b", foreground="white")
        self.lbl_s_donor_threshold_val.grid(row=1, column=2, padx=5, sticky="w")

        # Define btn_pausa before it's used
        self.btn_pausa = ttk.Button(
            frame_controles, 
            text="⏸ Pausar", # Initial text for play button
            command=self.toggle_pausa,
            style="Accent.TButton"
        )
        self.btn_pausa.grid(row=2, column=0, columnspan=3, sticky="ew", padx=5, pady=5) # Moved to a new row
        ...
        btns = [
            ...
            ("Nodo Alta S", self.agregar_nodo_alta_S, "🔥"), 
            ("Nodo Alta I", self.agregar_nodo_alta_I, "🌟"), 
        ...
        for i, (txt, cmd, emoji) in enumerate(btns):
            ...
            if i < 4: 
                ...
                b.grid(row=1, column=i-4, padx=2, pady=5, sticky="nsew") 
        ...
        self.motor.pausar_animacion(pausar=False) 
        self.btn_pausa.config(text="⏸ Pausar") 
        self.actualizar_s_donor_threshold() 
    ...
    def actualizar_velocidad(self, *args):
        ...
        if hasattr(self, 'motor') and hasattr(self.motor, 'campo'):
            ...
            self.lbl_velocidad_val.config(text=f"{self.velocidad.get():.1f}x") 
    ...
    def actualizar_s_donor_threshold(self, *args):
        """Actualiza el tope de entropía del campo de la simulación."""
        if hasattr(self, 'motor') and hasattr(self.motor, 'campo'):
            new_threshold = self.s_donor_threshold_var.get()
            self.motor.campo.S_donor_threshold = new_threshold
            self.lbl_s_donor_threshold_val.config(text=f"{new_threshold:.1f}") 
            self.lbl_status_bar.config(text=f"Tope S: {new_threshold:.1f} | Nodos: {len(self.motor.campo.nodos)}")
    ...
    def agregar_nodo_alta_S(self):
        """Añade un nuevo nodo de ALTA ENTROPÍA en una posición aleatoria dentro del canvas."""
        x = random.randint(50, self.motor.campo.ancho - 50)
        y = random.randint(50, self.motor.campo.alto - 50)
        
        S_val = random.uniform(2.0, 3.0) 
        I_val = random.uniform(0.1, 0.5) 
        
        self.motor.campo.nodos.append(Nodo(x, y, S=S_val, I=I_val))
        self.motor.dibujar()
        self.motor.actualizar_metricas()
        self.actualizar_contador_nodos_en_status_bar()
        self.lbl_status_bar.config(text=f"Nodo de alta Entropía añadido. Nodos: {len(self.motor.campo.nodos)}")

    def agregar_nodo_alta_I(self):
        """Añade un nuevo nodo de ALTA INFORMACIÓN en una posición aleatoria dentro del canvas."""
        x = random.randint(50, self.motor.campo.ancho - 50)
        y = random.randint(50, self.motor.campo.alto - 50)
        
        S_val = random.uniform(0.1, 0.5) 
        I_val = random.uniform(2.0, 3.0) 
        
        self.motor.campo.nodos.append(Nodo(x, y, S=S_val, I=I_val))
        self.motor.dibujar()
        self.motor.actualizar_metricas()
        self.actualizar_contador_nodos_en_status_bar()
        self.lbl_status_bar.config(text=f"Nodo de alta Información añadido. Nodos: {len(self.motor.campo.nodos)}")
    ...
    def cargar_patron_ejemplo(self):
        ...
        patron_proporcional = [
            {"x_norm": 0.15, "y_norm": 0.15, "S": 1.0, "I": 2.0}, 
            {"x_norm": 0.35, "y_norm": 0.25, "S": 2.0, "I": 1.0}, 
            {"x_norm": 0.55, "y_norm": 0.10, "S": 1.5, "I": 1.5}, 
            {"x_norm": 0.25, "y_norm": 0.40, "S": 0.8, "I": 2.2}, 
            {"x_norm": 0.65, "y_norm": 0.30, "S": 2.2, "I": 0.8}, 
            {"x_norm": 0.80, "y_norm": 0.18, "S": 1.0, "I": 1.0}, 
            {"x_norm": 0.20, "y_norm": 0.65, "S": 0.5, "I": 2.5}, 
            {"x_norm": 0.40, "y_norm": 0.75, "S": 2.5, "I": 0.5}, 
            {"x_norm": 0.60, "y_norm": 0.55, "S": 1.2, "I": 1.8}, 
            {"x_norm": 0.85, "y_norm": 0.90, "S": 1.8, "I": 1.2}  
        ]
    ...
    def crear_archivo_ejemplo(self):
        ...
        try:
            patron_proporcional = [
                {"x_norm": 0.10, "y_norm": 0.10, "S": 1.0, "I": 2.0}, 
                {"x_norm": 0.30, "y_norm": 0.20, "S": 2.0, "I": 1.0}, 
                {"x_norm": 0.50, "y_norm": 0.05, "S": 1.5, "I": 1.5}, 
                {"x_norm": 0.70, "y_norm": 0.35, "S": 0.8, "I": 2.2}, 
                {"x_norm": 0.90, "y_norm": 0.15, "S": 2.2, "I": 0.8}, 
                {"x_norm": 0.15, "y_norm": 0.50, "S": 1.0, "I": 1.0}, 
                {"x_norm": 0.45, "y_norm": 0.60, "S": 0.5, "I": 2.5}, 
                {"x_norm": 0.75, "y_norm": 0.80, "S": 2.5, "I": 0.5}, 
                {"x_norm": 0.25, "y_norm": 0.90, "S": 1.2, "I": 1.8}, 
                {"x_norm": 0.55, "y_norm": 0.45, "S": 1.8, "I": 1.2}  
            ]
            
            datos_ejemplo_abs = []
            for n_prop in patron_proporcional:
