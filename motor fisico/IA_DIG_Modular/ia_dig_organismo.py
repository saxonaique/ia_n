...
class DIGVisualizerApp(tk.Tk):
    ...
    def apply_configuration(self, show_message=True):
        """Aplica los ajustes de la GUI al motor de la IA."""
        try:
            # Validar y obtener valores de la GUI
            width = int(self.field_width_var.get())
            height = int(self.field_height_var.get())
            max_iter = int(self.max_iterations_var.get())
            fp_interval = int(self.fingerprint_interval_var.get())
            max_patterns = int(self.max_patterns_var.get())
            sim_threshold = float(self.similarity_threshold_var.get())

            # Aplicar al Metamodulo y sus componentes
            new_shape = (height, width)
            self.metamodulo.sensor_module.field_shape = new_shape
            self.metamodulo.core_nucleus.field_shape = new_shape
            self.metamodulo.max_iterations = max_iter
            self.metamodulo.fingerprint_interval = fp_interval
            
            learning_memory = self.metamodulo.fingerprint_system.learning_memory
            learning_memory.max_patterns = max_patterns
            learning_memory.similarity_threshold = sim_threshold
            
            self.plugin_manager.plugin_dir = self.plugin_dir_var.get()

            if show_message:
                messagebox.showinfo("Éxito", "Configuración aplicada.\nAlgunos cambios pueden requerir reiniciar la entrada.")
            print("[Config] Ajustes aplicados exitosamente.")
            
            # Refrescar la vista con un campo vacío del nuevo tamaño
            self.process_text()

        except ValueError:
            messagebox.showerror("Error de Validación", "Por favor, asegúrate de que todos los campos numéricos contengan números válidos.")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo aplicar la configuración: {e}")

    def save_configuration(self):
        """Guarda la configuración actual en config.json."""
        try:
            settings = {
                "field_width": self.field_width_var.get(),
                "field_height": self.field_height_var.get(),
                "max_iterations": self.max_iterations_var.get(),
                "fingerprint_interval": self.fingerprint_interval_var.get(),
                "max_patterns": self.max_patterns_var.get(),
            ...
            messagebox.showinfo("Guardado", "La configuración ha sido guardada en config.json.")
        except Exception as e:
    ...
    def load_configuration(self):
        ...
        if os.path.exists("config.json"):
            try:
                ...
                # Cargar valores en las variables de Tkinter
                self.field_width_var.set(settings.get("field_width", "64"))
                self.field_height_var.set(settings.get("field_height", "64"))
                self.max_iterations_var.set(settings.get("max_iterations", "2000"))
                self.fingerprint_interval_var.set(settings.get("fingerprint_interval", "1000"))
                self.max_patterns_var.set(settings.get("max_patterns", "100"))
                self.similarity_threshold_var.set(settings.get("similarity_threshold", "0.85"))
                self.plugin_dir_var.set(settings.get("plugin_dir", "plugins"))
                self.log_dir_var.set(settings.get("log_dir", "logs"))
                
                print("[Config] Configuración cargada desde config.json.")
                # Aplicar los ajustes cargados sin mostrar mensaje al usuario
                self.apply_configuration(show_message=False)
            except Exception as e:
                print(f"[Config] WARN: No se pudo cargar o aplicar config.json: {e}")

    def restore_defaults(self):
        """Restaura todos los ajustes de configuración a sus valores por defecto."""
        if messagebox.askyesno("Confirmar", "¿Restaurar todos los ajustes a los valores predeterminados?"):
            self.field_width_var.set("64")
            self.field_height_var.set("64")
            self.max_iterations_var.set("2000")
            self.fingerprint_interval_var.set("1000")
            self.max_patterns_var.set("100")
            self.similarity_threshold_var.set("0.85")
            self.plugin_dir_var.set("plugins")
            self.log_dir_var.set("logs")
    ...
    def export_configuration(self):
        ...
        if not filepath:
            return
        try:
            # Recolectar todas las variables StringVar para guardar
            settings = {
                "field_width": self.field_width_var.get(), "field_height": self.field_height_var.get(),
                "max_iterations": self.max_iterations_var.get(), "fingerprint_interval": self.fingerprint_interval_var.get(),
                "max_patterns": self.max_patterns_var.get(), "similarity_threshold": self.similarity_threshold_var.get(),
                "plugin_dir": self.plugin_dir_var.get(), "log_dir": self.log_dir_var.get()
            }
            with open(filepath, 'w') as f:
                json.dump(settings, f, indent=4)
            messagebox.showinfo("Exportado", f"Configuración exportada a\n{filepath}")
        except Exception as e:
            messagebox.showerror("Error", f"No se pudo exportar la configuración: {e}")

    def browse_plugin_dir(self):
        directory = filedialog.askdirectory(title="Seleccionar Directorio de Plugins")
        if directory:
